 ...
 */
namespace Nethues\Job\Ui\Component\Listing\Column;
...
{
    /** Url path */
    const CMS_URL_PATH_EDIT = 'job/application/edit';
    const CMS_URL_PATH_DELETE = 'job/application/delete';
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                // echo '</pre>';
                // exit;
                if (isset($item['id_job'])) {
                    $item[$name]['edit'] = [
                        'href' => $this->urlBuilder->getUrl($this->editUrl, ['id_job' => $item['id_job']]),
                        'label' => __('Edit'),
                    ];
                    $title = $this->getEscaper()->escapeHtml($item['candi_email']);
                    $item[$name]['delete'] = [
                        'href' => $this->urlBuilder->getUrl(self::CMS_URL_PATH_DELETE, ['id_job' => $item['id_job']]),
                        'label' => __('Delete'),
                        'confirm' => [
